...
void TestDemImport() {
    ...
    // A DEM is untrusted third-party binary — a file the user downloaded from
    // USGS or Copernicus. The decoder is the one place in the engine that
    // parses such input, so these cover both correctness and hostility.
    Titan::TerrainEngine e;
    e.Initialize(MakeParams(1, 0, 128));

    // --- Uncompressed little-endian int16 strips, built here ---------------
    auto buildTiff = [](int w, int h, const std::vector<int16_t>& px, bool bigEndian) {
        std::vector<uint8_t> b;
        auto u16 = [&](uint16_t v) {
            if (bigEndian) { b.push_back(v >> 8); b.push_back(v & 0xff); }
            else { b.push_back(v & 0xff); b.push_back(v >> 8); }
        };
        auto u32 = [&](uint32_t v) {
            if (bigEndian) { b.push_back(v >> 24); b.push_back((v >> 16) & 0xff);
                             b.push_back((v >> 8) & 0xff); b.push_back(v & 0xff); }
            else { b.push_back(v & 0xff); b.push_back((v >> 8) & 0xff);
                   b.push_back((v >> 16) & 0xff); b.push_back(v >> 24); }
        };
        b.push_back(bigEndian ? 'M' : 'I');
        b.push_back(bigEndian ? 'M' : 'I');
        u16(42);
        u32(8);
        const int tagCount = 10;
        const uint32_t dataOff = 8 + 2 + 12 * tagCount + 4;
        u16(tagCount);
        auto tag = [&](uint16_t id, uint16_t type, uint32_t count, uint32_t value) {
            u16(id); u16(type); u32(count);
            if (type == 3) { u16(static_cast<uint16_t>(value)); u16(0); }
            else u32(value);
        };
        tag(256, 3, 1, static_cast<uint32_t>(w));   // width
        tag(257, 3, 1, static_cast<uint32_t>(h));   // height
        tag(258, 3, 1, 16);                          // bits per sample
        tag(259, 3, 1, 1);                           // no compression
        tag(262, 3, 1, 1);                           // photometric
        tag(273, 4, 1, dataOff);                     // strip offset
        tag(277, 3, 1, 1);                           // samples per pixel
        tag(278, 3, 1, static_cast<uint32_t>(h));    // rows per strip
        tag(279, 4, 1, static_cast<uint32_t>(w * h * 2)); // strip bytes
        tag(339, 3, 1, 2);                           // signed samples
        u32(0);                                      // next IFD
        for (int16_t v : px) u16(static_cast<uint16_t>(v));
        return b;
    };

    const int w = 24, h = 24;
    std::vector<int16_t> px(static_cast<size_t>(w) * h);
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            px[static_cast<size_t>(y) * w + x] = static_cast<int16_t>(x * 20 - 200 + y * 5);
        }
    }
    const int16_t expectLo = -200, expectHi = static_cast<int16_t>(23 * 20 - 200 + 23 * 5);

    for (bool be : {false, true}) {
        auto file = buildTiff(w, h, px, be);
        const int side = e.DecodeDem(file.data(), file.size());
        Check(side == w, be ? "big-endian TIFF decodes" : "little-endian TIFF decodes");
        Check(std::fabs(e.DemMinElevation() - expectLo) < 0.5f &&
              std::fabs(e.DemMaxElevation() - expectHi) < 0.5f,
              be ? "big-endian elevations are exact" : "little-endian elevations are exact");
        Check(e.DemSourceWidth() == w && e.DemSourceHeight() == h,
              "source dimensions are reported");
    }

    // --- Deflate, against the hand-written inflate -------------------------
    // The riskiest code in the decoder: a full canonical-Huffman inflate
    // written from scratch because the engine carries no dependencies.
    {
        static const unsigned char kDeflateTiff[] = {
            ,
        };
        const int side = e.DecodeDem(kDeflateTiff, sizeof(kDeflateTiff));
        Check(side == 16, "deflate-compressed TIFF decodes");
        Check(std::fabs(e.DemMinElevation() - (-300.0f)) < 0.5f &&
              std::fabs(e.DemMaxElevation() - 420.0f) < 0.5f,
              "inflate reproduces the exact elevations");
        Check(AllFinite(e.DemField()), "decoded field holds no NaN/Inf");
    }

    // --- SRTM .hgt: raw big-endian int16, square, no header ----------------
    {
        const int side = 32;
        std::vector<uint8_t> hgt;
        for (int y = 0; y < side; ++y) {
            for (int x = 0; x < side; ++x) {
                const int16_t v = static_cast<int16_t>(x * 7 + y * 3);
                hgt.push_back(static_cast<uint8_t>(v >> 8));
                hgt.push_back(static_cast<uint8_t>(v & 0xff));
            }
        }
        Check(e.DecodeDem(hgt.data(), hgt.size()) == side, "square .hgt decodes from its length");
        Check(std::fabs(e.DemMaxElevation() - (31 * 7 + 31 * 3)) < 0.5f,
              ".hgt elevations are exact");

        // -32768 is SRTM's void marker. Treating it as terrain punches a 32 km
        // pit through the map and wrecks the normalization for every other cell.
        hgt[0] = 0x80; hgt[1] = 0x00;
        e.DecodeDem(hgt.data(), hgt.size());
        Check(e.DemMinElevation() > -1000.0f, "SRTM void markers are not read as elevation");
    }

    // --- Voids are interpolated, not flattened ------------------------------
    // A high tile with a hole in it: radar shadow behind a ridge is exactly
    // where real SRTM voids live. Zeroing the hole would be a 3 km pit here,
    // and would report a minimum elevation no ground sample produced.
    {
        const int side = 32;
        std::vector<int16_t> src(static_cast<size_t>(side) * side);
        for (int y = 0; y < side; ++y)
            for (int x = 0; x < side; ++x)
                src[static_cast<size_t>(y) * side + x] =
                    static_cast<int16_t>(3000 + x * 10 + y * 4);

        // A 6x6 void blob, wide enough that the fill has to spread inward.
        for (int y = 12; y < 18; ++y)
            for (int x = 12; x < 18; ++x)
                src[static_cast<size_t>(y) * side + x] = -32768;

        std::vector<uint8_t> hgt;
        for (int16_t v : src) {
            hgt.push_back(static_cast<uint8_t>(static_cast<uint16_t>(v) >> 8));
            hgt.push_back(static_cast<uint8_t>(v & 0xff));
        }
        Check(e.DecodeDem(hgt.data(), hgt.size()) == side, "a tile with voids still decodes");
        Check(AllFinite(e.DemField()), "no void survives as NaN in the field");
        Check(e.DemMinElevation() > 2900.0f,
              "a void does not drag the reported minimum below the real terrain");

        // The hole sat in terrain running 3168-3220 m. The fill must land in
        // that neighbourhood rather than at the tile mean or at zero.
        const std::vector<float>& f = e.DemField();
        const float span = e.DemMaxElevation() - e.DemMinElevation();
        float lo = 1e30f, hi = -1e30f;
        for (int y = 12; y < 18; ++y) {
            for (int x = 12; x < 18; ++x) {
                const float m = e.DemMinElevation() + f[static_cast<size_t>(y) * side + x] * span;
                lo = std::min(lo, m);
                hi = std::max(hi, m);
            }
        }
        std::printf("        void patch filled to %.0f-%.0f m (neighbours ~3168-3220)\n", lo, hi);
        Check(lo > 3120.0f && hi < 3270.0f, "voids are filled from the terrain around them");
    }

    // --- Hostile input ------------------------------------------------------
    auto rejects = [&](const std::vector<uint8_t>& bytes) {
        try { e.DecodeDem(bytes.data(), bytes.size()); return false; }
        catch (const std::exception&) { return true; }
    };
    Check(rejects({}), "empty input is rejected");
    Check(rejects({'I','I'}), "a two-byte 'TIFF' is rejected");
    Check(rejects({'I','I',42,0,0,0,0,0}), "a TIFF with a null IFD offset is rejected");
    Check(rejects({'x','y','z','w','1','2','3','4','5'}), "a non-square unknown file is rejected");

    // Every truncation of a real file must be reported, never crash or read
    // out of bounds — this is the case a corrupt download actually produces.
    {
        auto file = buildTiff(w, h, px, false);
        int survived = 0;
        for (size_t n = 1; n < file.size(); n += 13) {
            try { e.DecodeDem(file.data(), n); } catch (const std::exception&) {}
            ++survived;
        }
        Check(survived > 40, "every truncated prefix is handled without crashing");
    }
    std::printf("\n");
}
...
void TestLayerResolutionIndependence() {
    ...
    auto measure = [](int size, const char* layer) -> Stats {
        ...
        } else if (std::strcmp(layer, "thermal") == 0) {
            Titan::ThermalParams tp;
    ...
    for (const char* layer : {"blur", "fluvial", "hydraulic", "thermal"}) {
    ...
    // Thermal's pass count is scaled by 1/cellSize so a pass is a fixed world
    // distance rather than one cell, but that scaling is capped
    // (kThermalPassScaleCap) because the cost is passes * size^2. Past the cap
    // it under-settles rather than hanging, so check the cap actually binds
    // and still leaves sane terrain.
    {
        Titan::TerrainEngine e;
        Titan::TerrainParams p = MakeParams(12345, 1, 256);
        p.cellSize = 1.0f / 64.0f; // far past the cap
        e.Initialize(p);
        e.GenerateHeightmap();
        Titan::ThermalParams tp;
        e.ApplyThermalWeathering(4, tp);
        float lo = 0, hi = 0;
        e.HeightRange(lo, hi);
        Check(std::isfinite(hi) && hi > 0.0f && AllFinite(e.BedrockMap()),
              "thermal stays bounded and sane past the pass-scaling cap");
    }
    std::printf("\n");
}
